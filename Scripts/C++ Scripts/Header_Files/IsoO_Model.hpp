#ifndef IsoO_Model_hpp
#define IsoO_Model_hpp
...
{
public:
    Iso_Opacity_Giant_Planet(double mc, double me, double rp, double pd, double Td, double kd, double lp, int num_p, bool grid_given, std::vector<double> m_grid, std::array<double, 2> p_ana_c);
    ~Iso_Opacity_Giant_Planet();
    std::vector< std::vector<double> > getVariables(void);
    std::pair< std::vector<int>, std::vector< std::pair<int,int> > > getInfo(void);
    void printVariable(std::vector<double> variable);
    std::vector< std::vector<double> > Planet_Formation(std::string Rad_or_Conv);
...
private:
    ...
    std::vector<double> m, r, p, T, k, P, s, dP_dr_num, dP_dr_ana;
