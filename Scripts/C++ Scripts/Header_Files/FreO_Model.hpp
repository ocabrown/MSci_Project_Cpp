#ifndef FreO_Model_hpp
#define FreO_Model_hpp

#include <vector>
#include <array>
#include <string>

class Fre_Opacity_Giant_Planet
{
public:
    Fre_Opacity_Giant_Planet(double mc, double me, double rp, double pd, double Td, double kd, double lp, double met, double acc_k, int num_p, bool grid_given, std::vector<double> m_grid, std::array<double, 2> p_ana_c);
...
private:
    // Inputs:
    const double mc, me, rp, pd, Td, kd, lp, met, acc_k;        // Core mass, Envelope mass, Planetary radius, Disc density, Disc temperature, Disc opacity, Planet luminosity, Metallicity
    ...
    double Z;                   // Metallicity [log([M/H]/[M/H]_solar)]
