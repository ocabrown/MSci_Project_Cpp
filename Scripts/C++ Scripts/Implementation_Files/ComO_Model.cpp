...
Com_Opacity_Giant_Planet::Com_Opacity_Giant_Planet(double mc, double me, double rp, double pd, double Td, double kd, double lp, double ad, double d_to_g, double acc_k, int num_p, bool grid_given, std::vector<double> m_grid, std::array<double, 2> p_ana_c) : mc(mc), me(me), rp(rp), pd(pd), Td(Td), kd(kd), lp(lp), ad(ad), d_to_g(d_to_g), acc_k(acc_k), num_p(num_p), grid_given(grid_given), m_grid(m_grid), p_ana_c(p_ana_c)                                     // Custom constructor
{
...
Com_Opacity_Giant_Planet::~Com_Opacity_Giant_Planet(){}       // Destructor


std::vector< std::vector<double> > Com_Opacity_Giant_Planet::getVariables(void)
{
    return {m, r, p, T, k, P, s};
}


std::pair< std::pair< std::vector<int>, std::vector< std::pair<int,int> > >, bool > Com_Opacity_Giant_Planet::getInfo(void)
{
    return {{rad_or_conv, boundary_i}, broken};
}


void Com_Opacity_Giant_Planet::printVariable(std::vector<double> variable)
{
...
std::vector< std::vector<double> > Com_Opacity_Giant_Planet::Planet_Formation(void)
{
    for (int i = hi; i > lo; i--)
    {
...
//        }
        ...
        k[i-1] = Combined_Opacity(a_dust, dust_to_gas, P[i-1], T[i-1]);
        ...
        if (abs(Grad_Rad) < abs(Grad_Adia))     // Radiative
        {
            ...
            auto g_func = [this, i, B](double A)
            {
                ...
                double F = Combined_Opacity(a_dust, dust_to_gas, P[i-1], A);
...
std::array<double, 2> Com_Opacity_Giant_Planet::DensityFit(void)
{
