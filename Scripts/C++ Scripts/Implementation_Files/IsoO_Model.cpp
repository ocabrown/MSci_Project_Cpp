...
Iso_Opacity_Giant_Planet::Iso_Opacity_Giant_Planet(double mc, double me, double rp, double pd, double Td, double kd, double lp, int num_p, bool grid_given, std::vector<double> m_grid, std::array<double, 2> p_ana_c) : mc(mc), me(me), rp(rp), pd(pd), Td(Td), kd(kd), lp(lp), num_p(num_p), grid_given(grid_given), m_grid(m_grid), p_ana_c(p_ana_c)                                     // Custom constructor
{
...
Iso_Opacity_Giant_Planet::~Iso_Opacity_Giant_Planet(){}       // Destructor


std::vector< std::vector<double> > Iso_Opacity_Giant_Planet::getVariables(void)
{
    return {m, r, p, T, k, P, s, dP_dr_num, dP_dr_ana};
}


std::pair< std::vector<int>, std::vector< std::pair<int,int> > > Iso_Opacity_Giant_Planet::getInfo(void)
{
    return {rad_or_conv, boundary_i};
}


void Iso_Opacity_Giant_Planet::printVariable(std::vector<double> variable)
{
...
std::vector< std::vector<double> > Iso_Opacity_Giant_Planet::Planet_Formation(std::string Rad_or_Conv)
{
    for (int i = hi; i > lo; i--)
    {
        ...
        else if (Rad_or_Conv == "Purely Convective")
        {
            T[i-1] = T[i] * pow(P[i-1]/P[i], (gamma-1.)/gamma);
        }
        
        else if (Rad_or_Conv == "Both")
        {
    ...
    variables = {m, r, p, T, k, P, s, dP_dr_num, dP_dr_ana};
    
    return variables;
}


std::vector<double> Iso_Opacity_Giant_Planet::L2norm(void)
{
    std::vector<double> m_(variables[0].begin()+lo, variables[0].begin()+hi);
    std::vector<double> r_(variables[1].begin()+lo, variables[1].begin()+hi);
    std::vector<double> p_(variables[2].begin()+lo, variables[2].begin()+hi);
    std::vector<double> T_(variables[3].begin()+lo, variables[3].begin()+hi);
    std::vector<double> k_(variables[4].begin()+lo, variables[4].begin()+hi);
    std::vector<double> P_(variables[5].begin()+lo, variables[5].begin()+hi);
    std::vector<double> s_(variables[6].begin()+lo, variables[6].begin()+hi);
    std::vector<double> dP_dr_num_(variables[7].begin()+lo, variables[7].begin()+hi);
    std::vector<double> dP_dr_ana_(variables[8].begin()+lo, variables[8].begin()+hi);
    ...
    L2norm_vars = {L2norm_dP_dr, min_dP_dr};
    
    return L2norm_vars;
}


std::array<double, 2> Iso_Opacity_Giant_Planet::DensityFit(void)
{
